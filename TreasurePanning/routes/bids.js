...
router.get('/itemCurrentBid/:id',function(req,res){
  var collection = db.get('bids');
  collection.find({itemID: req.params.id}, function(err, bids){
    if((err)||bids.length==0){
      var itemCollecion=db.get('items');
      itemCollecion.findOne({ _id:req.params.id},function(err,item){
        if(err){
         res.status(404).json("item not found");
       }else{
        res.json({"username":"No Bidder yet","bidPrice":item.startPrice});
      }});
    }else{
...
router.get('/user/', function(req, res) {
 ...
 router.post('/',function(req, res){
   var itemCollecion=db.get('items');
   itemCollecion.findOne({ _id:req.body.itemID},function(err,item){
     console.log(item);
     if(err){
      res.status(404).json("item not found");
      }else{
